...
import distrax  # JAX-native reimplementation of a subset of TensorFlow Probability (TFP)
import jaxmarl
from jaxmarl.wrappers.baselines import MPELogWrapper as LogWrapper 
import matplotlib.pyplot as plt
import hydra    # facilitate configuration management
from omegaconf import OmegaConf # hierarchical configuration library
import wandb    # weights and biases

class ActorCritic(nn.Module):   # this is identical to PureJAXRL PPO :)
...
class Transition(NamedTuple):   # identical to PureJAXRL PPO
    done: jnp.ndarray
    action: jnp.ndarray
    value: jnp.ndarray
    reward: jnp.ndarray
...
# here starts some IPPO_FF_MPE-specific code!

def batchify(x: dict, agent_list, num_actors):  # Each key in the dictionary (represented by 'a' in the agent_list) corresponds to an agent, and the associated value is that agent's data or observation.
    max_dim = max([x[a].shape[-1] for a in agent_list]) # x[a].shape[-1] accesses the size of the last dimension of the array stored in the dictionary x under the key a
    def pad(z, length): # The pad function is used to pad an array z with zeros to match a specified length in its last dimension
...
# here ends some IPPO_FF_MPE-specific code
